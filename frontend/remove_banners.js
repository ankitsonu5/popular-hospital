...
for (const subdir of subdirs) {
  ...
  if (fs.existsSync(filePath)) {
    ...
    const heroImageRegex = /<div className=\"absolute inset-0 z-0\">\s*<Image[\s\S]*?\/>\s*<div className=\"absolute inset-0 [^\"]+\" \/>\s*<\/div>/g;
    ...
    if (divRegex.test(content)) {
      ...
      if (match.includes('<Image') && match.includes('images/wellness_packages')) {
