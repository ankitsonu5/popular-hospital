...
const OPD_DAYS = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section
        ...
        style={{ background: 'linear-gradient(135deg, #1a4a6b 0%, #2d7a9a 60%, #3aaccc 100%)' }}
      >
        <div
        ...
        <div className="relative z-10 mx-auto max-w-[1366px] px-6">
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-3 font-heading">Doctor Profile</h1>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-12">
              ...
              <div className="lg:col-span-4 flex flex-col items-center bg-gray-50/80 border-r border-gray-100 p-10">
                <div className="relative rounded-xl overflow-hidden shadow-md bg-white border border-gray-200 mb-6" style={{ height: '350px', width: '280px' }}>
                  {displayImage ? (
                    <Image
                      src={displayImage}
                      alt={displayName}
                    ...
                    <div className="flex items-center justify-center h-full bg-blue-50">
                      <svg className="w-32 h-32 text-blue-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <h2 className="text-lg font-bold text-[#3b82f6] text-center leading-tight">{displayName}</h2>
                {displayQualification && (
                  <p className="text-gray-500 text-sm text-center mt-2 leading-relaxed px-2">{displayQualification}</p>
                )}
                {displayDesignation && (
                  <p className="text-gray-700 text-sm font-bold uppercase tracking-wide mt-2 text-center">{displayDesignation}</p>
                )}
                {displayFee != null && (
                  <div className="mt-5 bg-blue-50 rounded-lg px-4 py-3 text-center w-full">
                    <p className="text-sm text-gray-500">Consultation Fee</p>
                    <p className="text-blue-600 font-bold text-xl">₹{displayFee}</p>
                  </div>
                )}
              </div>
              ...
              <div className="lg:col-span-8 p-10 lg:p-12">
                ...
                <div className="mb-6 pb-6 border-b border-gray-100">
                  ...
                  {displayBio && (
                    <p className="text-gray-500 text-base mt-3 leading-relaxed">{displayBio}</p>
                  )}
                </div>
                ...
                {opdBranches.map((branch, bIdx) => (
                  <div key={bIdx} className={bIdx > 0 ? 'mt-8' : ''}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-[#3b82f6] font-semibold text-sm">- OPD Timings</span>
                    </div>
                        ...
                        <thead>
                          <tr className="bg-[#0b1c43] text-white">
                            {branch.schedule.map(s => (
                              <th key={s.day} className="py-4 px-5 text-center font-semibold text-sm tracking-wide">
                                {s.day}
                              </th>
                            ))}
                          </tr>
                            ...
                            {branch.schedule.map(s => (
                              <td
                                ...
                                className={`py-5 px-4 text-center text-sm font-medium leading-snug ${
                                  s.timing === '-' ? 'text-gray-400' : 'text-gray-700'
                                }`}
                              >
                                {s.timing}
                              </td>
