...
import { 
  Calendar, Search, RefreshCw, Phone, MapPin, 
  Clock, CheckCircle, ArrowLeft, MoreVertical, 
  Trash2, User, ChevronLeft, ChevronRight, Square, Stethoscope, AlertTriangle
} from 'lucide-react';
...
export default function BookingsAdminPage() {
  ...
  const filtered = bookings.filter((b) => {
    return (
      ...
      (b.doctor && b.doctor.name.toLowerCase().includes(search.toLowerCase())) ||
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 bg-white">
          ...
          {selected ? (
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                title={selected.status === 'pending' ? 'Cannot delete pending booking' : 'Delete Booking'}
              >
            ...
            <div className="flex items-center gap-1">
               <div className="p-2 hover:bg-gray-100 rounded-md cursor-pointer"><Square className="w-4 h-4 text-gray-400" /></div>
                <button onClick={fetchBookings} className="p-2 hover:bg-gray-100 rounded-md" title="Refresh"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /></button>
                <button className="p-2 hover:bg-gray-100 rounded-md"><MoreVertical className="w-4 h-4" /></button>
             </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center text-gray-500">
             <span className="text-xs mr-4 hidden sm:inline">{filtered.length > 0 ? `1-${filtered.length}` : '0'} of {bookings.length}</span>
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="bg-gray-50 rounded-2xl p-6 mb-6 flex items-start gap-4 border border-gray-100">
              ...
              <div className="flex-1">
                <h3 className="text-xl font-bold text-gray-900">{selected.patient_name}</h3>
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-sm text-gray-600 font-medium">
                  <span className="flex items-center gap-1.5"><Phone className="w-4 h-4 text-gray-400" /> {selected.patient_phone}</span>
                  {selected.patient_email && <span className="flex items-center gap-1.5"><User className="w-4 h-4 text-gray-400" /> {selected.patient_email}</span>}
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
               <div className="border border-gray-100 rounded-2xl p-5 bg-white shadow-sm">
                  <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Booking Information</div>
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      <Calendar className="w-5 h-5 text-indigo-500 shrink-0" />
                      <div>
                        <div className="text-[11px] font-semibold text-gray-400">APPOINTMENT DATE</div>
                        <div className="text-sm font-semibold text-gray-800">{selected.slot_date}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
               ...
               <div className="border border-gray-100 rounded-2xl p-5 bg-white shadow-sm">
                  <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Hospital Details</div>
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      <Stethoscope className="w-5 h-5 text-blue-500 shrink-0" />
                      <div>
                        <div className="text-[11px] font-semibold text-gray-400">ASSIGNED DOCTOR</div>
                        <div className="text-sm font-semibold text-gray-800">{selected.doctor?.name || 'N/A'}</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <MapPin className="w-5 h-5 text-rose-500 shrink-0" />
                      <div>
                         <div className="text-[11px] font-semibold text-gray-400">BRANCH LOCATION</div>
                         <div className="text-sm font-semibold text-gray-800">{selected.branch?.name || 'N/A'}</div>
                      </div>
                    </div>
                  </div>
               </div>
            ...
            {selected.notes && (
              <div className="bg-amber-50 rounded-2xl p-6 border border-amber-100">
                ...
                <p className="text-gray-700 text-sm whitespace-pre-wrap leading-relaxed">{selected.notes}</p>
              </div>
            )}
        ...
        <div className="flex-1 overflow-y-auto w-full bg-white divide-y divide-gray-100">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 p-8 text-center">
              ...
              <p className="text-[15px] font-medium text-gray-500">No bookings found</p>
            ...
            filtered.map((booking) => (
              ...
              >
                ...
                <div className="hidden sm:flex items-center gap-4 flex-shrink-0 w-[220px]">
                  <div className="p-1 hover:bg-gray-200 rounded text-gray-300 cursor-pointer" onClick={(e) => { e.stopPropagation(); }}><Square className="w-4 h-4" /></div>
                  {!booking.isRead ? <User className="w-4 h-4 text-red-500 fill-red-500 shrink-0" /> : <User className="w-4 h-4 text-blue-400 shrink-0" />}
                  <span className={`${!booking.isRead ? 'font-bold text-gray-900' : 'font-semibold text-gray-700'} truncate pr-2 tracking-tight`}>{booking.patient_name}</span>
                </div>

                {/* Mobile top-row */}
                <div className="flex justify-between w-full sm:hidden mb-1">
                   <div className={`${!booking.isRead ? 'font-bold text-gray-900' : 'font-semibold text-gray-700'} flex items-center gap-2`}>
                     {!booking.isRead ? <User className="w-3.5 h-3.5 text-red-500 fill-red-500" /> : <User className="w-3.5 h-3.5 text-blue-500" />}
                     {booking.patient_name}
                   </div>
                   <span className="text-xs font-medium text-gray-500 shrink-0">{formatDate(booking.createdAt)}</span>
                </div>
                ...
                <div className="hidden sm:flex items-center gap-3 flex-shrink-0 min-w-[180px] justify-end pr-4">
                  ...
                  <button 
                    onClick={(e) => { 
                      e.stopPropagation(); 
                      if (booking.status !== 'pending') promptDelete(booking._id); 
                    }}
                    ...
                    title={booking.status === 'pending' ? 'Cannot delete pending booking' : 'Delete'}
                  >
