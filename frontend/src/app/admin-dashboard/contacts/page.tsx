...
export default function GmailContactsPage() {
  ...
  const filtered = contacts.filter((c) => {
    ...
    const matchTab = activeTab === 'primary' ? true : c.status === (activeTab === 'unread' ? 'new' : 'read');
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-100 bg-white">
        <div className="flex items-center gap-1 text-gray-500">
          {selected ? (
            ...
            <div className="flex items-center gap-1">
               <div className="p-2 hover:bg-gray-100 rounded-md cursor-pointer"><Square className="w-4 h-4 text-gray-400" /></div>
                <button onClick={fetchContacts} className="p-2 hover:bg-gray-100 rounded-md" title="Refresh"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} /></button>
          ...
          {selected && (
            <div className="flex items-center gap-1 border-l pl-2 ml-1">
              ...
              <button className="p-2 hover:bg-gray-100 rounded-md"><Mail className="w-4 h-4 text-gray-600" /></button>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center text-gray-500">
             <span className="text-xs mr-4 hidden sm:inline">{filtered.length > 0 ? `1-${filtered.length}` : '0'} of {contacts.length}</span>
                  ...
                  return (
                    <div 
                      ...
                      className={`group flex items-center px-4 py-2 border-b hover:shadow-md transition-all cursor-pointer ${
                        isNew ? 'bg-white font-bold' : 'bg-gray-50/50 font-normal opacity-85'
                      } ${
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Star className={`w-4 h-4 text-gray-300 group-hover:text-gray-400 hover:text-yellow-400`} />
                      </div>

                      {/* Name */}
                      <div className={`w-32 sm:w-48 shrink-0 truncate text-sm ${isNew ? 'text-gray-900' : 'text-gray-700'}`}>
                      ...
                      <div className="flex-1 min-w-0 truncate text-sm flex items-center gap-2">
                        <div className="flex-1 truncate">
                          <span className={isNew ? 'text-gray-900' : 'text-gray-800'}>
                            Appointment Request: {contact.department || 'General Inquiry'}
                          </span>
                          <span className="text-gray-500 ml-2 font-normal">
                            - {contact.message ? contact.message.substring(0, 100) : 'New appointment contact details submitted...'}
                          </span>
                        </div>
                        {contact.isInternational && (
                      ...
                      <div className="w-28 shrink-0 flex items-center justify-end h-full">
                        <div className={`text-xs ${isNew ? 'text-gray-900 font-bold' : 'text-gray-500'} group-hover:hidden transition-opacity`}>
           ...
           <div className="px-6 py-6 border-b border-gray-100">
              <div className="flex items-center justify-between mb-2">
                 <h2 className="text-2xl font-normal text-gray-800 flex items-center gap-3">
                   ...
                   <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded uppercase tracking-wider font-bold">Inbox</span>
                 ...
                 <div className="flex items-center gap-2 text-gray-500">
                    <button className="p-2 hover:bg-gray-100 rounded-full"><RefreshCw className="w-4 h-4" /></button>
                    <button className="p-2 hover:bg-gray-100 rounded-full"><Settings className="w-4 h-4" /></button>
                 </div>
              </div>
           </div>
           ...
           <div className="flex-1 overflow-y-auto">
             <div className="max-w-4xl mx-auto px-6 py-8">
                ...
                <div className="bg-white rounded-2xl p-8 border border-gray-200 shadow-[0_1px_3px_rgba(0,0,0,0.1)] mb-8">
                   <h3 className="text-sm font-bold text-gray-400 uppercase tracking-[0.2em] mb-8 pb-4 border-b border-gray-100">Appointment Request Summary</h3>
                   
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-y-10 gap-x-12 mb-10">
                      <DetailRow label="Patient Name" value={selected.name} />
                      <DetailRow label="Phone Number" value={selected.phone} isPhone />
                      <DetailRow label="Preferred Date" value={selected.date ? new Date(selected.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' }) : 'Not specified'} />
                      <DetailRow label="Timing Slot" value={selected.timing || 'Not specified'} />
                      <DetailRow label="Department" value={selected.department || 'Not specified'} />
                      <DetailRow label="Branch/Location" value={selected.location || 'Not specified'} />
                      {selected.isInternational && (
                        <>
                          <DetailRow label="Country" value={selected.country || 'Not specified'} />
                          <DetailRow label="Age" value={selected.age || 'Not specified'} />
                        </>
                      )}
                   </div>
                   ...
                   {selected.message && (
                     <div className="mt-10 pt-10 border-t border-gray-100">
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-6">Patient Message</p>
...
function DetailRow({ label, value, isPhone = false }: { label: string; value: string; isPhone?: boolean }) {
  return (
    <div>
      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 opacity-70">{label}</p>
      {isPhone ? (
        <a href={`tel:${value}`} className="text-blue-600 font-semibold text-lg hover:underline transition-all decoration-blue-200 underline-offset-4">{value}</a>
      ) : (
