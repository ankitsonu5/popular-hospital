...
import {
  Stethoscope, Building2, CalendarCheck, Users,
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#0b1c43] to-[#1e3a8a] p-6 sm:p-8 text-white">
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/60 text-sm sm:text-base max-w-xl">
            Manage your hospital data from one place. Here&apos;s a quick overview of your system.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="space-y-2">
            {[
              { label: 'Add New Doctor', href: '/admin-dashboard/doctors', icon: Stethoscope, color: 'text-blue-600 bg-blue-50' },
              { label: 'Add New Branch', href: '/admin-dashboard/branches', icon: Building2, color: 'text-emerald-600 bg-emerald-50' },
              { label: 'Manage Bookings', href: '/admin-dashboard/bookings', icon: CalendarCheck, color: 'text-orange-600 bg-orange-50' },
              { label: 'Update Content', href: '/admin-dashboard/content', icon: TrendingUp, color: 'text-purple-600 bg-purple-50' },
            ].map((action) => {
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin-dashboard/bookings" className="text-xs text-[#0d9488] font-medium hover:underline">
              View All →
            </Link>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left py-3 px-2 text-xs font-semibold text-gray-500 uppercase tracking-wider">Patient</th>
                    <th className="text-left py-3 px-2 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden sm:table-cell">Doctor</th>
                    <th className="text-left py-3 px-2 text-xs font-semibold text-gray-500 uppercase tracking-wider hidden md:table-cell">Date</th>
                ...
                <tbody>
                  {recentBookings.map((b: any, i: number) => (
                    <tr key={i} className="border-b border-gray-50 last:border-0">
                      ...
                      <td className="py-3 px-2 hidden sm:table-cell text-gray-600">{b.doctor?.name || '-'}</td>
