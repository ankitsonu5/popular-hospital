...
import { Plus, Pencil, Trash2, Search, Loader2, Calendar, Camera, ImageIcon, ExternalLink, Sparkles } from 'lucide-react';
...
function EventsList() {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_URL}/cms/events`, { headers: getHeaders() });
      if (res.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`${API_URL}/cms/events/${id}`, { method: 'DELETE', headers: getHeaders() });
      fetchData();
    } catch (e) {
      console.error(e);
    }
    ...
    <div className="max-w-[1366px] mx-auto pb-20 font-sans tracking-tight">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
        <div className="space-y-1">
          ...
          <h1 className="text-3xl font-bold text-gray-900 tracking-tighter">Events Gallery</h1>
          <p className="text-sm text-gray-500 font-medium tracking-tight">Manage photo galleries and visuals for hospital events.</p>
        </div>
      ...
      {isLoading && events.length === 0 ? (
        <div className="py-40 flex flex-col items-center justify-center space-y-4">
           ...
           <p className="text-[10px] font-bold text-gray-300 uppercase tracking-widest text-center italic">Developing Negatives...</p>
        </div>
      ) : (
          ...
          <div className="overflow-x-auto">
                    ...
                    <td className="px-8 py-7">
                      <div className="flex items-center gap-5">
                        <div className="w-14 h-14 rounded-2xl bg-gray-50 overflow-hidden border border-gray-100 shadow-sm relative group-hover:scale-105 transition-transform duration-500 overflow-hidden shrink-0">
                          {item.thumbnail ? (
                             <img src={getImageUrl(item.thumbnail)} alt={item.title} className="w-full h-full object-cover transition-transform duration-700" title={item.title} />
                          ) : (
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1 opacity-60">
                             <span className="text-[10px] font-bold tracking-widest border border-gray-100 px-2 py-0.5 rounded uppercase">ID: {item._id.slice(-6)}</span>
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-7 whitespace-nowrap">
                       <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center border border-gray-100 text-blue-600 group-hover:bg-blue-50 transition-colors">
                             <span className="text-xs font-black">{item.gallery?.length || 0}</span>
                          </div>
                          <span className="text-[10px] text-gray-400 font-extrabold uppercase tracking-widest">Photos</span>
                       </div>
                    </td>
            ...
            {filteredEvents.length === 0 && (
              <div className="py-24 flex flex-col items-center justify-center px-10">
                 ...
                 <h3 className="font-extrabold text-gray-400 text-lg uppercase tracking-tight">No Events Found</h3>
              </div>
            )}
          </div>
        </div>
