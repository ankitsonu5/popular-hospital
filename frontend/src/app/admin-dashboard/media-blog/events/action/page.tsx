          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white p-8 sm:p-10 rounded-2xl shadow-sm border border-gray-200">
              <div className="flex items-center justify-between mb-8">
                ...
                <span className="text-xs font-bold text-gray-400 font-medium">
                  Total: {existingGallery.length + galleryFiles.length} Photos
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4">
                ...
                {existingGallery.map((img, i) => (
                  <div
                    key={i}
                    className="relative aspect-square rounded-xl overflow-hidden group border border-gray-200 shadow-sm transition-transform hover:scale-[1.02]"
                  >
                    <Image
                      src={getImageUrl(img)}
                      alt="Gallery"
                    ...
                    <button
                      ...
                      onClick={() =>
                        setExistingGallery(
                          existingGallery.filter((_, idx) => idx !== i),
                        )
                      }
                      className="absolute top-2 right-2 p-1.5 bg-red-500/90 hover:bg-red-600 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-md"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                    <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-gray-900/60 text-white text-[8px] font-bold uppercase rounded-md">
                      Existing
                    </div>
                  </div>
                ))}
                ...
                {galleryPreviews.map((pre, i) => (
                  <div
                    key={i}
                    className="relative aspect-square rounded-xl overflow-hidden group border border-blue-200 shadow-sm transition-transform hover:scale-[1.02]"
                  >
                    <Image
                      src={pre}
                      alt="New Preview"
                    ...
                    <button
                      ...
                      className="absolute top-2 right-2 p-1.5 bg-gray-900/90 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-md"
                    >
                      <X className="w-3.5 h-3.5" />
                    </button>
                    <div className="absolute bottom-2 left-2 px-2 py-0.5 bg-blue-600 text-white text-[8px] font-bold uppercase rounded-md">
                      New Upload
                    </div>
                  </div>
                ))}
                ...
                <label className="relative aspect-square rounded-xl border-2 border-dashed border-gray-200 hover:border-blue-400 bg-gray-50 flex items-center justify-center cursor-pointer transition-all hover:bg-blue-50/30 group">
                  <div className="text-center group-hover:scale-110 transition-transform">
                    ...
                    <span className="text-[10px] font-bold text-gray-400 group-hover:text-blue-500 uppercase">
                      Add Photo
                    </span>
                  </div>
                  <input
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200">
              ...
              <p className="text-[10px] text-gray-400 mt-4 leading-relaxed font-medium">
                This image will be used as the primary thumbnail across the
                website.
              </p>
            </div>
          </div>
