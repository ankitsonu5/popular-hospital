            ...
            {filteredBlogs.map((item) => (
              <div key={item._id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-4">
                <div className="flex gap-4">
                  <div className="w-20 h-20 rounded-xl bg-slate-50 border border-indigo-100 overflow-hidden shrink-0">
                    {item.image ? (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                       <span className="px-2 py-0.5 bg-indigo-50 text-indigo-600 rounded-full text-[8px] font-bold uppercase tracking-widest">{item.category || 'Health'}</span>
                ...
                <div className="flex gap-2 pt-4 border-t border-gray-50">
                  <Link href={`/admin-dashboard/media-blog/blog/action?id=${item._id}`} target="_blank" className="flex-1 flex items-center justify-center gap-2 py-2 bg-blue-50 text-blue-600 rounded-xl font-bold text-xs">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50 font-semibold text-gray-600 border-b border-gray-100">
                  <tr>
                    <th className="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Blog Post</th>
                    ...
                    <tr key={item._id} className="hover:bg-gray-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 rounded-lg bg-indigo-50 overflow-hidden shrink-0 border border-indigo-100">
                            {item.image ? (
                               <img src={item.image.startsWith('http') ? item.image : `${API_URL}${item.image}`} alt={item.title} className="w-full h-full object-cover" />
                            ) : (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                         <span className="text-xs font-semibold text-[#0d9488] bg-teal-50 px-3 py-1 rounded-full border border-teal-100">{item.category || 'Health'}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <Link href={`/admin-dashboard/media-blog/blog/action?id=${item._id}`} target="_blank" className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg">
