  ...
  return (
    <div>
      ...
      <p className="mt-1 text-gray-600">
        Add and edit doctor profiles. Non-technical staff can update names,
        qualifications, and branch assignments.
      </p>
      {error && (
        <p className="mt-4 rounded bg-red-100 p-2 text-sm text-red-700">
          {error}
        </p>
      )}
      <button type="button" onClick={openNew} className="btn-primary mt-6">
        Add Doctor
      </button>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    speciality_id: parseInt(e.target.value, 10),
                  })
                }
                className="mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm"
                required
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, branch_ids: e.target.value })
                }
                className="mt-1 w-full rounded border border-gray-300 px-3 py-2 text-sm"
                placeholder="1, 2, 3"
              />
              <p className="mt-1 text-xs text-gray-500">
                Branches:{" "}
                {branches.map((b) => `${b._id}: ${b.name}`).join(", ")}
              </p>
            </div>
            <div className="sm:col-span-2">
              <label className="block text-sm font-medium text-gray-700">
              ...
              {list.map((d) => (
                <tr key={d.id}>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-600">
                    {d.speciality_name || d.speciality_id}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">
