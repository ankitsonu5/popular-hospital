    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api-backend/blogs/search?q=${encodeURIComponent(query)}`,
        );
        if (res.ok) {
              ...
              results.map((r, i) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-bold text-[#1a3a5c] line-clamp-2 leading-tight">
                      {r.title}
                    </p>
                    {r.category && (
                      <p className="text-xs text-[#E85222] font-semibold mt-1">
                        {r.category}
                      </p>
                    )}
                  </div>
        ...
        <div className="bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-slate-100">
          ...
          <ul className="flex flex-col w-full">
            {displayedRecent.map((p, idx) => (
              <li
                key={p._id || p.id || idx}
                className="border-b border-gray-100 py-4 first:pt-0 last:border-0 last:pb-0"
              >
                <Link
                  href={`/blog/${p.slug}`}
                  className="text-[#1a3a5c] text-[15px] font-medium hover:text-[#E85222] transition-colors leading-[1.6]"
                >
                  {p.title}
                </Link>
              </li>
      ...
      <div className="bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-slate-100">
        ...
        <ul className="flex flex-col w-full">
          {displayedCategories.map((cat, i) => (
            <li
              key={i}
              className="border-b border-gray-100 py-3 first:pt-0 last:border-0 last:pb-0"
            >
              <Link
                href="#"
                className="flex justify-between items-center text-[15px] text-[#1a3a5c] font-medium hover:text-[#E85222] transition-colors group"
              >
                <span>{cat._id}</span>
                {cat.count > 0 && (
