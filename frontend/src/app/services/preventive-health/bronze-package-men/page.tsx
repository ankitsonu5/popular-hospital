...
export const metadata: Metadata = {
  title: 'Bronze Package Men | Preventive Health Check Up | Popular Hospital',
  description: 'Detailed information and items covered under the Bronze Package for Men.',
};

export default function BronzePackageMenPage() {
  const consultations = [
    'GENERAL PHYSICIAN',
    'GENERAL SURGEON'
  ];
  ...
  const investigations = [
    ...
    'URINE ANALYSIS', 'USG WHOLE ABDOMEN', 'X-RAY CHEST PA VIEW', 'ECG'
  ];
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1366px] px-6">
          <div className="max-w-4xl">
            ...
            <nav className="flex items-center text-sm md:text-base text-white/90 font-medium" aria-label="Breadcrumb">
              ...
              <span className="mx-2 text-red-600 font-bold">|</span>
              <span className="text-white">Bronze Package Men</span>
            </nav>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col md:flex-row items-center md:items-center justify-between gap-10 mb-20">
            ...
            <div className="w-full md:w-[70%] bg-white rounded-[32px] shadow-[0_8px_40px_rgba(0,0,0,0.06)] border border-gray-100 flex overflow-hidden group">
                <div className="w-[160px] sm:w-[220px] relative bg-[#f5f5f7] flex-shrink-0">
                    <Image
                        src="/images/health-packages/bronze_package_men.jpg"
                        alt="Bronze Package Men"
                        fill
             ...
             <div className="mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {consultations.map((item, id) => (
                        <div key={id} className="bg-white border border-gray-100/80 p-5 rounded-2xl text-[14px] text-[#1d1d1f] font-medium uppercase shadow-sm flex items-center gap-4 group/item hover:border-blue-100 transition-colors"><div className="w-2 h-2 rounded-full bg-blue-500/50 group-hover/item:bg-blue-500 transition-colors"></div><div className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]"></div>
                           {item}
                        </div>
                    ))}
                </div>
