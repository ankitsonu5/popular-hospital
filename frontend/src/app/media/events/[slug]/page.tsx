...
import { X, ChevronLeft, ChevronRight, Loader2, Calendar, LayoutGrid, Maximize2 } from "lucide-react";
  ...
  if (!event) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white px-4 text-center">
        ...
        <Link href="/media/events" className="text-indigo-600 font-bold hover:underline underline-offset-4">Back to all events</Link>
      </div>
    );
  }
      ...
      <section className="max-w-[1280px] mx-auto px-6 sm:px-8 lg:px-12 py-16">
        ...
        {event.gallery.length === 0 && (
          <div className="text-center py-20 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-100">
            <p className="text-gray-400 italic">No gallery photos added for this event yet.</p>
          </div>
        )}
      </section>
        ...
        <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-sm flex flex-col animate-in fade-in duration-300">
           ...
           <div className="flex items-center justify-between p-6 text-white z-10 w-full">
              <div className="flex flex-col">
                <span className="text-xs font-bold text-indigo-400 uppercase tracking-widest mb-1">{event.title}</span>
                <span className="text-[10px] text-white/50">{selectedImageIndex + 1} / {event.gallery.length}</span>
              </div>
              <button onClick={closeLightbox} className="p-3 bg-white/10 hover:bg-white/20 rounded-full transition-all">
                <X className="w-6 h-6" />
              </button>
           </div>
           ...
           <div className="relative flex-1 flex items-center justify-center p-4 sm:p-12 overflow-hidden">
              <button onClick={prevImg} className="absolute left-4 sm:left-12 p-4 bg-black/40 hover:bg-indigo-600/80 text-white rounded-full transition-all z-20 backdrop-blur-md">
