  ...
  return (
    <main className="min-h-screen bg-white font-sans overflow-x-hidden">
      ...
      <section className="py-20 bg-white">
        <div className="mx-auto w-full max-w-[1366px] px-6 text-center">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-[#0b1c43] mb-8 font-heading">Our Mission for a Better World</h2>
            <p className="text-gray-600 text-lg md:text-xl leading-relaxed mb-10 text-justify md:text-center">
              Our mission is to not only treat illnesses but also to foster a healthier, more equitable world through our comprehensive social responsibility initiatives. With a strong commitment to community outreach, we aim to make a positive impact on the lives of individuals and families across our region.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center mt-16">
              ...
              <div className="text-left space-y-6">
                 <div className="flex items-start gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-bold text-[#0b1c43]">Community Focused</h3>
                      <p className="text-gray-500">Tailoring our services to meet the specific needs of our local community.</p>
                    </div>
                 </div>
                 <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-full bg-emerald-500 flex items-center justify-center text-white font-bold flex-shrink-0">2</div>
                    <div>
                      <h3 className="text-xl font-bold text-[#0b1c43]">Sustainable Impact</h3>
                      <p className="text-gray-500">Creating long-term health solutions through education and awareness.</p>
                    </div>
                 </div>
                 <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-full bg-orange-500 flex items-center justify-center text-white font-bold flex-shrink-0">3</div>
                    <div>
                      <h3 className="text-xl font-bold text-[#0b1c43]">Equitable Access</h3>
                      <p className="text-gray-500">Breaking barriers to healthcare for underprivileged groups.</p>
                    </div>
                 </div>
              </div>
            </div>
      ...
      <section className="py-24 bg-gray-50">
        <div className="mx-auto w-full max-w-[1366px] px-6">
          <div className="text-center mb-16">
             <span className="text-blue-600 font-bold tracking-widest text-xs uppercase mb-3 block">Outreach</span>
             <h2 className="text-3xl md:text-5xl font-bold text-[#0b1c43] font-heading">Our Community Programs</h2>
             <div className="w-20 h-1.5 bg-blue-600 mx-auto mt-6 rounded-full" />
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {outreachPrograms.map((program, idx) => (
              <div key={idx} className="bg-white p-10 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-500 group border border-gray-100 hover:-translate-y-2">
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-8 transition-transform group-hover:rotate-12 ${program.color}`}>
                  {program.icon}
                </div>
                <h3 className="text-2xl font-bold text-[#0b1c43] mb-4 font-heading">{program.title}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 text-center text-white">
            {stats.map((stat, idx) => (
              <div key={idx} className="group">
                <div className="text-4xl mb-6 transform group-hover:scale-125 transition-transform duration-300">{stat.icon}</div>
                <div className="text-5xl font-black mb-2 font-heading text-blue-400">{stat.value}</div>
                <div className="text-blue-100 font-bold uppercase tracking-widest text-sm">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-32 bg-white">
        <div className="mx-auto w-full max-w-[1366px] px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div>
              <span className="text-blue-600 font-bold uppercase tracking-[0.2em] text-xs mb-4 block">Equity & Inclusion</span>
              ...
              <div className="space-y-6 text-gray-700 text-lg leading-relaxed text-justify">
                <p>
                  At Popular Hospital, we believe that every individual deserves access to high-quality healthcare, regardless of their socioeconomic background. By addressing health disparities and providing resources to underserved communities, we strive to create an equitable and healthy future for everyone.
                </p>
                <p>
                  Through our community programs and partnerships with local organizations, we ensure that healthcare services are accessible to all. We are building a future where your ability to pay never determines your right to live.
                </p>
              </div>
            </div>
            <div className="relative">
               ...
               <div className="absolute bottom-10 -left-10 bg-white p-8 rounded-2xl shadow-2xl max-w-xs border border-gray-100 hidden md:block">
                  <p className="text-blue-600 font-bold text-xl italic">"Making a positive impact on every life we touch."</p>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto w-full max-w-[1366px] px-6">
          <div className="bg-gradient-to-br from-[#0b1c43] to-[#042d55] rounded-[3rem] p-12 md:p-24 text-center text-white relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10 max-w-3xl mx-auto">
              <h2 className="text-3xl md:text-5xl font-bold mb-8 font-heading">Join Us in Making a Difference</h2>
              <p className="text-blue-100 text-lg md:text-xl mb-12 leading-relaxed">
                Whether through volunteering, donations, or simply spreading awareness, your support helps us continue to make a difference. Together, we can build a healthier world for future generations.
              </p>
      ...
      <footer className="py-12 bg-white text-center border-t border-gray-50">
        <p className="text-gray-400 text-sm">© Popular Hospital CSR Initiatives. Empowering Communities Since 1994.</p>
      </footer>
