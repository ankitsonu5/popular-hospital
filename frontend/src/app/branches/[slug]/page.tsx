...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description:
      branch.description ||
      `${branch.name}, ${branch.address}, ${branch.city}. Contact: ${branch.phone}.`,
  };
}
  ...
  return (
    <div className="container-narrow py-10">
      <nav className="mb-6 text-sm text-gray-600" aria-label="Breadcrumb">
        <Link href="/branches" className="hover:text-hospital-teal">
          Branches
        </Link>
      ...
      {branch.description && (
        <p className="mt-4 text-gray-600">{branch.description}</p>
      )}
      <div className="mt-8 grid gap-8 md:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-hospital-navy">
            Address & Contact
          </h2>
          <address className="mt-4 not-italic text-gray-600">
            ...
            <p>
              {branch.city}
              {branch.state ? `, ${branch.state}` : ""} {branch.pincode || ""}
            </p>
            {branch.phone && (
              <p className="mt-2">
                <a
