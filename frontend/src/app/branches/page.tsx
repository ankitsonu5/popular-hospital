  ...
  return (
    <div className="container-narrow py-10">
      ...
      <p className="mt-2 text-gray-600">
        Visit any of our branches for OPD, diagnostics, and specialist care.
        Select a branch for details and directions.
      </p>
        ...
        {Object.entries(byCity)
          ...
          .map(([city, list]) => (
            <section key={city} aria-labelledby={`branch-${city}`}>
              <h2
                id={`branch-${city}`}
                className="mb-4 text-xl font-semibold text-hospital-navy"
              >
                {city}
              </h2>
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {list.map((branch) => (
                  ...
                  >
                    <h3 className="font-semibold text-hospital-navy">
                      {branch.name}
                    </h3>
                    ...
                    {branch.phone && (
                      <p className="mt-2 text-sm font-medium text-hospital-teal">
                        {branch.phone}
                      </p>
                    )}
                    <span className="mt-3 inline-block text-sm font-medium text-hospital-teal">
                      View details →
                    </span>
                  </Link>
                ))}
