...
const mapping = [
  ...
  { keywords: [/gastroenterology/i, /gastrosurgery/i], slug: 'gastroenterology' },
  { keywords: [/plastic surgery/i], slug: 'burns-plastic-surgery' },
  { keywords: [/general surgery/i, /laparoscopy/i], slug: 'general-surgery' },
  { keywords: [/obgy/i, /gynaecology/i, /obstetrics/i, /dgoy/i], slug: 'gynaecology' },
  { keywords: [/gynecology/i], slug: 'gynecology' },
  { keywords: [/pediatrics/i, /neonatology/i, /pediatric/i], slug: 'pediatrics' },
  { keywords: [/internal medicine/i, /physician/i, /medicine/i], slug: 'general-medicine' },
  { keywords: [/anaesthesia/i, /pain/i], slug: 'pain-management' },
  { keywords: [/radiology/i, /radiodiagnosis/i, /radiotherapy/i, /imaging/i], slug: 'radiology' },
  { keywords: [/pathology/i, /microbiology/i, /laboratory/i], slug: 'pathology' },
...
async function run() {
  ...
  const specMap = {};
  specialities.forEach(s => { specMap[s.slug] = s._id; });
  ...
  for (const doc of doctors) {
    ...
    const currentSpec = specialities.find(s => s._id.toString() === doc.speciality?.toString());
    
    if (!currentSpec) {
      console.log(`Doctor ${doc.name} has broken speciality ${doc.speciality}. Qualification: ${q}`);
      ...
      if (matchedSlug && specMap[matchedSlug]) {
        await Doctor.findByIdAndUpdate(doc._id, { speciality: specMap[matchedSlug] });
