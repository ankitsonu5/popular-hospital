...
export const getAvailableSlots = async (req, res) => {
  try {
    ...
    if (!doctor_id || !branch_id || !date) {
      return res
        .status(400)
        .json({ error: "doctor_id, branch_id and date are required" });
    }

    // Validate ObjectIds
    if (
      !mongoose.Types.ObjectId.isValid(doctor_id) ||
      !mongoose.Types.ObjectId.isValid(branch_id)
    ) {
      return res
        .status(400)
        .json({ error: "Invalid doctor or branch ID format" });
    }
  ...
  try {
    ...
    if (!branch_id || !doctor_id || !slot_date || !Array.isArray(slots)) {
      return res.status(400).json({
        error: "branch_id, doctor_id, slot_date and slots array required",
      });
    }
    ...
    const operations = slots.map((slot_time) => ({
      updateOne: {
        filter: {
          branch: branch_id,
          doctor: doctor_id,
          slot_date: String(slot_date).trim(),
          slot_time: String(slot_time).trim(),
        },
