...
export const createEvent = async (req, res) => {
  try {
    ...
    const thumbnailPath = (filesObj.thumbnail && filesObj.thumbnail.length > 0) 
      ? `/uploads/events/${filesObj.thumbnail[0].filename}` 
      : (req.body.thumbnail || '');
      
    const galleryPaths = (filesObj.gallery || []).map(f => `/uploads/events/${f.filename}`);
...
export const updateEvent = async (req, res) => {
  try {
    ...
    // Handle Gallery
    const newGalleryFiles = (filesObj.gallery || []).map(f => `/uploads/events/${f.filename}`);
    let existingGallery = [];
    ...
    const item = await Event.findByIdAndUpdate(req.params.id, updates, { new: true });
    if (!item) return res.status(404).json({ error: 'Event not found' });
    res.json(item);
  } catch (error) {
    res.status(500).json({ error: error.message });
