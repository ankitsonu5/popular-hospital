...
const fileFilter = (req, file, cb) => {
  ...
  if (allowed.includes(ext)) {
    ...
    cb(new Error('Invalid file type. Only PDF, DOC, and Images allowed.'), false);
  }
};
...
export const submitApplication = async (req, res) => {
  try {
    ...
    const application = new Application({
      ...
      photoUrl: req.files['photo'] ? `/uploads/applications/${req.files['photo'][0].filename}` : undefined
    });
...
export const getApplicationById = async (req, res) => {
  try {
    const application = await Application.findById(req.params.id).populate('appliedFor');
...
export const toggleStarred = async (req, res) => {
  try {
    ...
    const application = await Application.findByIdAndUpdate(id, { isStarred }, { new: true }).populate('appliedFor');
    if (!application) return res.status(404).json({ message: 'Application not found' });
    res.json(application);
  } catch (err) {
    res.status(500).json({ message: 'Error updating starred status', error: err.message });
  }
};
...
export const toggleRead = async (req, res) => {
  try {
    ...
    const application = await Application.findByIdAndUpdate(id, { isRead }, { new: true }).populate('appliedFor');
    if (!application) return res.status(404).json({ message: 'Application not found' });
    res.json(application);
  } catch (err) {
